...
// TimelineBase is an object representing the database table.
...
var TimelineBaseColumns = struct {
	TimelineID  string
	UserID      string
	Name        string
	Description string
...
var TimelineBaseTableColumns = struct {
	TimelineID  string
	UserID      string
	Name        string
	Description string
...
// TimelineBaseRels is where relationship names are stored.
var TimelineBaseRels = struct {
	User string
}{
	User: "User",
}
...
func (r *timelineR) GetUser() *UserBase {
	if r == nil {
		return nil
	}
	return r.User
...
type (
	...
	TimelineBaseSlice []*TimelineBase
...
var (
	timelineType                 = reflect.TypeOf(&TimelineBase{})
...
func (q timelineQuery) One(ctx context.Context, exec boil.ContextExecutor) (*TimelineBase, error) {
	o := &TimelineBase{}
...
// All returns all TimelineBase records from the query.
func (q timelineQuery) All(ctx context.Context, exec boil.ContextExecutor) (TimelineBaseSlice, error) {
	var o []*TimelineBase

	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "entities: failed to assign all query results to TimelineBase slice")
	}

	return o, nil
}

// Count returns the count of all TimelineBase records in the query.
...
func (o *TimelineBase) User(mods ...qm.QueryMod) userQuery {
...
func (timelineL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTimelineBase interface{}, mods queries.Applicator) error {
	var slice []*TimelineBase
	var object *TimelineBase

	if singular {
		var ok bool
		object, ok = maybeTimelineBase.(*TimelineBase)
		if !ok {
			object = new(TimelineBase)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeTimelineBase)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeTimelineBase))
			}
		}
	} else {
		s, ok := maybeTimelineBase.(*[]*TimelineBase)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeTimelineBase)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeTimelineBase))
			}
		}
	}
...
func (o *TimelineBase) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *UserBase) error {
	var err error
	if insert {
...
// FindTimelineBase retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindTimelineBase(ctx context.Context, exec boil.ContextExecutor, timelineID types.TimelineID, selectCols ...string) (*TimelineBase, error) {
	timelineObj := &TimelineBase{}
...
func (o *TimelineBase) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
...
// Update uses an executor to update the TimelineBase.
// See boil.Columns.UpdateColumnSet documentation to understand column list inference for updates.
// Update does not automatically update the record in case of default values. Use .Reload() to refresh the records.
func (o *TimelineBase) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
...
func (o TimelineBaseSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) error {
	ln := int64(len(o))
	if ln == 0 {
		return nil
	}
...
// Delete deletes a single TimelineBase record with an executor.
// Delete will match against the primary key column to find the record to delete.
func (o *TimelineBase) Delete(ctx context.Context, exec boil.ContextExecutor) error {
	if o == nil {
		return errors.New("entities: no TimelineBase provided for delete")
	}
...
func (o TimelineBaseSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) error {
	if len(o) == 0 {
		return nil
	}
...
func (o *TimelineBase) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindTimelineBase(ctx, exec, o.TimelineID)
	if err != nil {
		return err
	}
...
func (o *TimelineBaseSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	if o == nil || len(*o) == 0 {
		return nil
	}

	slice := TimelineBaseSlice{}
	var args []interface{}
	for _, obj := range *o {
		pkeyArgs := queries.ValuesFromMapping(reflect.Indirect(reflect.ValueOf(obj)), timelinePrimaryKeyMapping)
		args = append(args, pkeyArgs...)
	...
	if err != nil {
		return errors.Wrap(err, "entities: unable to reload all in TimelineBaseSlice")
	}
...
// TimelineBaseExists checks if the TimelineBase row exists.
func TimelineBaseExists(ctx context.Context, exec boil.ContextExecutor, timelineID types.TimelineID) (bool, error) {
	var exists bool
...
// Exists checks if the TimelineBase row exists.
func (o *TimelineBase) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return TimelineBaseExists(ctx, exec, o.TimelineID)
}
